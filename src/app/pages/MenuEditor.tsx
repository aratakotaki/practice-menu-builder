...
export default function MenuEditor() {
  ...
  const handleSave = async (isNew: boolean, isSilent: boolean = false) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/make-server-791d0b68/menus`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 409) {
        ...
        if (!isSilent) {
          ...
          const overwriteRes = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/make-server-791d0b68/menus`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
